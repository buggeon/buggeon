...
function CardPreview({title, priority, status, boardId, cardId} : {title : string, priority : string, status : string, boardId : string, cardId : string}) {

    const navigate = useNavigate()

    return(
        <section className={styles.main} onClick={() => navigate(`${boardId}/cards/${cardId}`)}>
            <p>{title}</p>
            {
